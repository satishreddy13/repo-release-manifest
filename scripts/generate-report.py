...
def write_summary_sheet(wb, sprint, build_info, envs, generated_at, dep_warnings=None):
    ...
    warn_start = env_hdr + 4
    if dep_warnings:
        ws.row_dimensions[warn_start].height = 8  # spacer
        warn_hdr_row = warn_start + 1
        ws.merge_cells(f"A{warn_hdr_row}:D{warn_hdr_row}")
        c = ws[f"A{warn_hdr_row}"]
        c.value = "⚠  DEPENDENCY WARNINGS"
        c.fill  = hfill("C55A11")   # dark amber/orange
        c.font  = hfont(WHITE, bold=True, size=10)
        c.alignment = Alignment(horizontal="left", vertical="center", indent=1)
        ws.row_dimensions[warn_hdr_row].height = 18

        for wi, (team_label, dep_label) in enumerate(dep_warnings):
            r = warn_hdr_row + 1 + wi
            ws.merge_cells(f"A{r}:D{r}")
            c = ws[f"A{r}"]
            c.value = f"  {team_label}  depends on  {dep_label}  —  but {dep_label} is DEFERRED this sprint"
            c.fill  = hfill("FCE4D6")
            c.font  = hfont("C00000", bold=True, size=10)
            c.alignment = Alignment(horizontal="left", vertical="center", indent=2)
            ws.row_dimensions[r].height = 16

        footer_row = warn_hdr_row + len(dep_warnings) + 2
    else:
        footer_row = warn_start + 1
...
def write_teams_sheet(wb, teams, included_keys, deferred_keys, deferred_set=None):
    if deferred_set is None:
    ...
    for row_idx, key in enumerate(TEAM_ORDER, start=2):
        ...
        # Check if any listed dependency is deferred
        dep_list = [d.strip() for d in depends.split(",") if d.strip()]
        dep_broken = any(d in deferred_set for d in dep_list)
        dep_display = depends if depends else ""
        ...
        for col_idx, val in enumerate(row_data, start=1):
            ...
            if col_idx == 1:
                ...
                c.alignment = Alignment(horizontal="center", vertical="center")
            elif col_idx == 8:  # Depends On
                if dep_broken:
                    c.fill = hfill("FCE4D6")
                    c.font = hfont("C00000", bold=True, size=10)
                    c.value = f"⚠ {dep_display}"
                elif dep_display:
                    c.fill = hfill(AMBER_FILL)
                else:
    ...
    set_col_widths(ws, [26, 12, 10, 32, 18, 28, 38, 22, 38])
...
def main():
    ...
    for key in included_keys:
        deps_raw = teams[key].get("depends_on", "").strip()
        if not deps_raw:
            continue
        for dep in [d.strip() for d in deps_raw.split(",") if d.strip()]:
            if dep in deferred_set:
                dep_warnings.append((
                    TEAM_LABELS.get(key, key),
                    TEAM_LABELS.get(dep, dep),
                ))

    if dep_warnings:
        print("⚠️  DEPENDENCY WARNINGS:")
        for team_label, dep_label in dep_warnings:
            print(f"   {team_label} depends on {dep_label} — but {dep_label} is DEFERRED")
        print()
    ...
    write_summary_sheet(wb_out, sprint, build_info, envs, generated_at, dep_warnings)
    write_teams_sheet(wb_out, teams, included_keys, deferred_keys, deferred_set)
